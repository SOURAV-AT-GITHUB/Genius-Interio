...
export default function DesignSolutions() {
  ...
  const validIndex = slideNavItems.findIndex(
    (item) => item.toLowerCase().split(" ").join("-") === category
  );
  ...
  const checkScrollPosition = () => {
    if (slideNavRef.current) {
      ...
      const isAtEnd =
        ...
        container.scrollWidth - 1; // Added tolerance of -1px
  ...
  const handleClick = (title) => {
    dispatch({
      type: ADD_UPDATE_FORM_DATA,
      payload: {
        description: `I would like to know more about ${title} product`,
      },
    });
    navigate("/");
    setTimeout(() => {
      const contactSection = document.getElementById("home-get-in-touch");
      if (contactSection) {
  ...
  const getData = async () => {
    try {
      const response = await axios.get(
        `${DATABASE_URL}/categories/${category}.json`
      );
      if (response.data) {
        const images = Object.entries(response.data.images).map(
          ([id, rest]) => ({ ...rest })
        );
  ...
  useEffect(() => {
    if (
      ...
      const index = slideNavItems.findIndex(
        (item) => item.toLowerCase().split(" ").join("-") === category
      );
    ...
    <div className="p-3 lg:p-16 flex flex-col gap-4">
      ...
      <div className="relative">
        <button
          ...
          className={`${
            isAtStart && "hidden"
          }  absolute left-0 top-1 bg-white border rounded-full border-black`}
        >
        ...
        <div
          ref={slideNavRef}
          className="px-2 sm:px-8 py-1  w-full overflow-x-scroll flex gap-6 text-nowrap  hide-scrollbar"
        >
          {slideNavItems.map((item, index) => (
            <NavLink
              key={item}
              to={`/design-solutions/${item
                .toLowerCase()
                .split(" ")
                .join("-")}`}
            >
        ...
        <button
          ...
          className={`${
            isAtEnd && "hidden"
          }  absolute right-0 top-1 bg-white  border rounded-full border-black`}
        >
